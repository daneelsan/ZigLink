...
/* Disable the quoted include warning in Clang if it's on. It raises unnecessary
   warnings when linking component libraries & frameworks against the RTL
   framework on iOS and macOS. */
...
#define MArgument_getMSparseArray(marg)                                        \
  (*MArgument_getMSparseArrayAddress(marg))
#define MArgument_getMRawArray(marg) (*MArgument_getMRawArrayAddress(marg))
#define MArgument_getMNumericArray(marg)                                       \
  (*MArgument_getMNumericArrayAddress(marg))
